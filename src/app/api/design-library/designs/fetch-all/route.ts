...
function transformDesign(item: any) {
  // Customize as needed for your design schema
  return item;
}
...
async function fetchAllDesignsFromDynamoDB() {
  isFetching = true;
  fetchProgress = 0;
  fetchItems = [];
  fetchError = null;
  ...
  try {
    do {
      ...
      const items = (response.Items || []).map(transformDesign);
