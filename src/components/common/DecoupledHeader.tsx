...
interface DecoupledHeaderProps {
  ...
  analyticsBar?: React.ReactNode;
}

export default function DecoupledHeader({ columns, visibleColumns, onColumnsChange, analyticsBar }: DecoupledHeaderProps) {
  ...
  return (
    <div className="flex justify-between items-center w-full relative mb-2 gap-2 bg-white p-3 rounded shadow-sm">
      {/* Left: columns chips and settings */}
      ...
      {/* Right: analytics bar */}
      {analyticsBar && (
        <div className="flex items-center">{analyticsBar}</div>
      )}
    </div>
  );
}
